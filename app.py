...
selected_subreddit = st.sidebar.selectbox("Choose a subreddit", df["subreddit"].unique())
filtered_df = df[df["subreddit"] == selected_subreddit]

# Handle timestamp and avoid SettingWithCopyWarning
filtered_df = filtered_df.copy()
filtered_df["timestamp_utc"] = pd.to_datetime(filtered_df["timestamp_utc"], errors='coerce')
filtered_df["Year-Month"] = filtered_df["timestamp_utc"].dt.to_period("M").astype(str)

# Run sentiment classification if not present
if "sentiment" not in filtered_df.columns:
    st.info("Running sentiment classification...")
    filtered_df["sentiment"] = filtered_df["content"].apply(classify_text)

# Title
st.title("Reddit Cyberbullying Sentiment Analysis")

# Wordcloud
st.subheader("WordCloud")
all_text = " ".join(filtered_df["content"].dropna().astype(str).values)
...
fig = px.bar(sentiment_counts, x="Sentiment", y="Count", color="Sentiment", title="Sentiment Breakdown")
st.plotly_chart(fig)

# Time Series
st.subheader("Sentiment Over Time")
time_sentiment = filtered_df.groupby(["Year-Month", "sentiment"]).size().unstack().fillna(0)
fig = px.line(time_sentiment, title="Sentiment Trend Over Time")
st.plotly_chart(fig)

# Data Table
st.subheader("Data Table")
st.dataframe(filtered_df[["timestamp_utc", "subreddit", "content", "sentiment"]].sort_values("timestamp_utc", ascending=False))
