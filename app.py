...
def load_data():
    ...
    conn = sqlite3.connect(db_path)
    query = """
        SELECT id, title, text, url, score, comments, subreddit, timestamp_utc, label
        FROM reddit_labelled
    """
    df = pd.read_sql_query(query, conn)
    conn.close()
...
st.sidebar.write(f"📋 Total rows in DB: {len(df)}")
st.sidebar.write(f"🕒 Rows with valid timestamps: {df['timestamp_utc'].notna().sum()}")
...
col1, col2, col3, col4 = st.columns(4)
col1.metric("Total Posts", len(filtered_df))
col2.metric("% Bullying Posts", f"{(filtered_df['Bullying'].mean()) * 100:.1f}%" if not filtered_df.empty else "0%")
col3.metric("Most Active Subreddit", filtered_df['Subreddit'].mode().values[0] if not filtered_df.empty else "N/A")
col4.metric("Top Platform", filtered_df['Platform'].mode().values[0] if not filtered_df.empty else "N/A")
...
if not filtered_df.empty:
    ...
    st.warning("No data to show for the selected filters.")
