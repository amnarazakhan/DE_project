...
def load_data():
    ...
    # Convert timestamp (UNIX assumed)
    df['timestamp_utc'] = pd.to_datetime(df['timestamp_utc'], unit='s', errors='coerce')
...
# Add dummy label if not present
if 'label' in df.columns:
    df.rename(columns={'label': 'Bullying'}, inplace=True)
else:
    df['Bullying'] = 0
...
st.sidebar.write("📋 Total rows in DB:", len(df))
st.sidebar.write("🕒 Rows with valid timestamps:", df['timestamp_utc'].notna().sum())

# === Sidebar Filters ===
st.sidebar.header("🔍 Filter Data")

disable_filters = st.sidebar.checkbox("🚫 Disable Filters", value=False)
